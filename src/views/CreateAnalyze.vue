    ...
    methods: {
      ...
      */
      ...
      async getnft(page){
        ...
        var rule="";
        //var rule_num=1;
        var res = await axios.get(`https://mainnet-node.like.co/likechain/likenft/v1/collector?creator=like1t7ex3agu2lck34dn056asfgq5ch5hyqltufetf`);
        ...
        if(this.creatorlist.length>0){
          ...
          for(var i=0;i<this.creatorlist.length;i++){
            console.log(this.creatorlist[i].iscn_owner)
            var res = await axios.get(`https://mainnet-node.like.co/likechain/likenft/v1/collector?creator=${this.creatorlist[i].iscn_owner}`);
            //console.log(res.data)
            var collectors_num
            if(res.data.collectors){
              collectors_num=res.data.collectors.length
            }
            else{
              collectors_num=0
            }
            this.alllist.push({
              ...
              iscn_owner:this.creatorlist[i].iscn_owner,
              //stakeholders:res2.data.records[0].data.stakeholders.length,
              author:this.creatorlist[i].author,
              //iscn_owner_num:'',
